...
@Repository
class MessageRepositoryImpl(private val reactiveCassandraTemplate: ReactiveCassandraTemplate) : MessageRepository {

    private val logger = LoggerFactory.getLogger(this::class.java)

    override fun get(
        user1: String, user2: String, start: Instant, end: Instant
    ): Flux<Message> {
        val conversationId = UUIDUtils.getConversationId(user1, user2)
        val buckets = BucketUtils.bucketsInRange(start, end)

        return Flux.fromIterable(buckets).concatMap { bucket ->
            ...
            reactiveCassandraTemplate.select(query, Message::class.java)
    ...
    override fun store(
        ...
        return reactiveCassandraTemplate.insert(message).map { true }
    ...
    override fun delete(
        ...
        return Flux.fromIterable(buckets).flatMap { bucket ->
            val query = Query.query(
                where("conversation_id").`is`(conversationId),
                where("bucket").`is`(bucket),
                where("message_timestamp").gte(start),
            ...
            reactiveCassandraTemplate.delete(query, Message::class.java)
        }.then(Mono.just(true))
