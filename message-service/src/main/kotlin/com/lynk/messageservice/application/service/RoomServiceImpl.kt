...
import com.lynk.messageservice.infrastructure.outbound.kafka.KafkaEventPublisher
...
class RoomServiceImpl(
    ...
    override fun sendMessage(
        ...
        return memberByRoomRepository.getMemberById(senderId, roomId)
            .switchIfEmpty(Mono.error(RoomUnauthorizedException("Sender is not a member of this room"))).flatMap { _ ->
                    ...
                    .flatMap { success ->
                        if (success) {
                            ...
                            val roomMessageEvent = RoomMessageEvent(
                                messageId = roomMessage.key.messageId,
                            ...
                            eventPublisher.publish(roomMessageEvent)
                            reactiveRedisTemplate.convertAndSend(channel, payload)
                                .doOnSuccess { logger.info("Published message to Redis channel: $channel") }.then(
                                    dispatchOfflineNotifications(
                                        messageId, roomMessage, phoneNumber
                                    )
                                ).then(Mono.just(true))
                        } else {
                            Mono.just(false)
    ...
    private fun dispatchOfflineNotifications(
        messageId: UUID, message: RoomMessage, senderPhoneNumber: String
    ): Mono<Void> {
        return getRoomMembers(message.key.roomId).filter { it.memberId != message.sender_id }.flatMap { member ->
            presenceService.isUserOnline(member.memberId).flatMap { isOnline ->
                if (!isOnline) {
                    ...
                    val request = RoomMessageEvent(
                        messageId = messageId,
                        payload = RoomMessagePayload(message.content, null, message.key.roomId, senderPhoneNumber)
                    )
                    Mono.fromRunnable<Void> {
                        try {
                            kafkaEventPublisher.publish(request)
                        } catch (e: Exception) {
                            logger.error("Failed to send FCM to user ${member.memberId}", e)
                        }
                    }
