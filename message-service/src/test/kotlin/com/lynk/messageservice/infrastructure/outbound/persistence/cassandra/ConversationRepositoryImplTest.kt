...
class ConversationRepositoryImplTest {
    ...
    fun `store should save a conversation successfully`() {
        val result = conversationRepository.store("Hello there!", userId1.toString(), recipientId1)
    ...
    fun `store should handle empty message preview`() {
        val result = conversationRepository.store("", userId1.toString(), recipientId1)
        ...
        StepVerifier.create(
            ...
            .expectNextMatches { it.lastMessagePreview == "" }
            .verifyComplete()
    }
    ...
    fun `get should retrieve conversations within a specific time range`() {
        ...
        conversationRepository.store("Recent message", userId1.toString(), recipientId1).block()
        // This one is too old
    ...
    fun `get should return conversations in descending order of timestamp`() {
        val now = Instant.now()
        conversationRepository.store("Older Message", userId1.toString(), recipientId1).block()
        Thread.sleep(10) // ensure distinct timestamps
        conversationRepository.store("Newer Message", userId1.toString(), recipientId2).block()


        val conversations = conversationRepository.get(userId1.toString(), now.minus(1, ChronoUnit.MINUTES), now.plus(1, ChronoUnit.MINUTES))

        StepVerifier.create(conversations)
            .expectNextMatches { it.lastMessagePreview == "Newer Message" }
            .expectNextMatches { it.lastMessagePreview == "Older Message" }
            .verifyComplete()
    }
    ...
    fun `get should return empty flux for a valid user but empty time range`() {
        conversationRepository.store("A message", userId1.toString(), recipientId1).block()
    ...
    fun `delete should remove conversations within a time range`() {
        val now = Instant.now()
        conversationRepository.store("To be deleted", userId1.toString(), recipientId1).block()
        val oldConversation = ConversationEntity(
            key = com.lynk.messageservice.infrastructure.outbound.persistence.cassandra.entity.ConversationKey(
                userId = userId1,
                lastActivityTimestamp = now.minus(2, ChronoUnit.HOURS),
    ...
    fun `delete should only remove conversations for the specified user`() {
        val now = Instant.now()
        conversationRepository.store("User1's message", userId1.toString(), recipientId1).block()
        conversationRepository.store("User2's message", userId2.toString(), recipientId1).block()
