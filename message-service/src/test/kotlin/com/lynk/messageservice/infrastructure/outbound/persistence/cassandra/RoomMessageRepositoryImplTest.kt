...
class RoomMessageRepositoryImplTest {
    ...
    fun `getMessagesByRoomId should retrieve messages within a time range`() {
        val now = Instant.now()
        roomMessageRepository.saveMessage(roomId, "Message 1", senderId, UUID.randomUUID(), timestamp = now.minusSeconds(10)).block()
        ...
        StepVerifier.create(messages.collectList())
            .assertNext {
    ...
    fun `getMessagesByRoomId should retrieve messages across different time buckets`() {
        ...
        roomMessageRepository.saveMessage(roomId, "Recent Message", senderId, UUID.randomUUID(), timestamp = now).block()
        roomMessageRepository.saveMessage(roomId, "Old Message", senderId, UUID.randomUUID(), timestamp = lastMonth).block()
