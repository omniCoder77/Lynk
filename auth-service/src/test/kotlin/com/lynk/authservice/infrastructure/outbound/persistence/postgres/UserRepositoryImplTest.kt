...
class UserRepositoryImplTest {
    ...
    fun setup() {
        ...
        """.trimIndent()).fetch().rowsUpdated().block()
    }
    ...
    fun `persist should save user and return userId`() {
        ...
        val user = User(userId.toString(), "testuser", phoneNumber = "1234567890", firstName = "testuser", lastName = "testuser")
    ...
    fun `setTotpSecret should update totp secret and return affected rows`() {
        val userId = UUID.randomUUID()
        val initialUser = User(userId.toString(), "testuser", phoneNumber = "1234567890", firstName = "testuser", lastName = "testuser")
        ...
        StepVerifier.create(
            ...
            .expectNextMatches { it.totpSecret == totpSecret }
            .verifyComplete()
    }
