...
class FCMChatService {
    ...
    private fun buildChatData(
        chatId: String, senderId: String, messageId: String, messageType: MessageType, senderAvatar: String? = null
    ): Map<String, String> {
        ...
        senderAvatar?.let { data["senderAvatar"] = it }
    ...
    private fun getWebConfig(
        body: String,
        senderAvatar: String? = "/default-avatar.png",
        chatId: String,
        senderId: String,
        messageId: String,
        ...
        return WebpushConfig.builder().setNotification(
            WebpushNotification.builder().setBody(body).setIcon(senderAvatar).setTag("chat_${chatId}").setRenotify(true)
                .setVibrate(intArrayOf(200, 100, 200)).addAllActions(
                    mutableListOf(
                        WebpushNotification.Action(
                            "mark_read", "Mark as Read", "/icons/check.png"
                        )
                    )
                ).putAllCustomData(buildChatData(chatId, senderId, messageId, messageType, senderAvatar)).build()
        ).setFcmOptions(
